...
pub fn call_function(name: &str, args: &[String]) -> Option<String> {
    match name {
        ...
        "strstr" => {
            let s = args.first()?;
            let sub = args.get(1)?;
            Some(s.find(sub.as_str()).map_or(String::new(), |i| (i + 1).to_string()))
        }
        ...
        "hex" => {
            ...
            let digits = args.get(1).and_then(|s| s.parse::<usize>().ok()).unwrap_or(0);
            if digits > 0 {
...
mod tests {
    ...
    #[test]
    fn left() {
        assert_eq!(call_function("left", &s(&["hello", "3"])), Some("hel".into()));
    }
    ...
    #[test]
    fn left_unicode() {
        assert_eq!(call_function("left", &s(&["\u{00e9}l\u{00e8}ve", "3"])), Some("\u{00e9}l\u{00e8}".into()));
    }

    #[test]
    fn right() {
        assert_eq!(call_function("right", &s(&["hello", "3"])), Some("llo".into()));
    }
    ...
    fn pad_left_already_wider() {
        assert_eq!(call_function("pad", &s(&["hello", "3"])), Some("hello".into()));
    }

    #[test]
    fn pad_right() {
        assert_eq!(call_function("pad_right", &s(&["42", "5"])), Some("42   ".into()));
    }

    #[test]
    fn padcut() {
        assert_eq!(call_function("padcut", &s(&["hi", "5"])), Some("   hi".into()));
        assert_eq!(call_function("padcut", &s(&["hello world", "5"])), Some("hello".into()));
    }

    #[test]
    fn padcut_right() {
        assert_eq!(call_function("padcut_right", &s(&["hi", "5"])), Some("hi   ".into()));
        assert_eq!(call_function("padcut_right", &s(&["hello world", "5"])), Some("hello".into()));
    }
    ...
    #[test]
    fn trim() {
        assert_eq!(call_function("trim", &s(&["  hello  "])), Some("hello".into()));
    }
    ...
    #[test]
    fn caps() {
        assert_eq!(call_function("caps", &s(&["hello world"])), Some("Hello World".into()));
    }
    ...
    #[test]
    fn abbr() {
        assert_eq!(call_function("abbr", &s(&["Aphex Twin"])), Some("AT".into()));
        assert_eq!(
    ...
    #[test]
    fn substr() {
        assert_eq!(call_function("substr", &s(&["hello world", "6", "11"])), Some("world".into()));
    }

    #[test]
    fn substr_out_of_bounds() {
        assert_eq!(call_function("substr", &s(&["hi", "0", "100"])), Some("hi".into()));
    }

    #[test]
    fn insert() {
        assert_eq!(call_function("insert", &s(&["hello", "XX", "2"])), Some("heXXllo".into()));
    }

    #[test]
    fn repeat() {
        assert_eq!(call_function("repeat", &s(&["ab", "3"])), Some("ababab".into()));
    }
    ...
    #[test]
    fn stripprefix_the() {
        assert_eq!(call_function("stripprefix", &s(&["The Beatles"])), Some("Beatles".into()));
    }

    #[test]
    fn stripprefix_a() {
        assert_eq!(call_function("stripprefix", &s(&["A Perfect Circle"])), Some("Perfect Circle".into()));
    }

    #[test]
    fn stripprefix_no_match() {
        assert_eq!(call_function("stripprefix", &s(&["Radiohead"])), Some("Radiohead".into()));
    }
    ...
    fn swapprefix_no_match() {
        assert_eq!(call_function("swapprefix", &s(&["Radiohead"])), Some("Radiohead".into()));
    }
    ...
    #[test]
    fn fix_eol() {
        assert_eq!(call_function("fix_eol", &s(&["line1\nline2\rline3"])), Some("line1 line2 line3".into()));
    }
    ...
    #[test]
    fn strchr_found() {
        assert_eq!(call_function("strchr", &s(&["hello", "l"])), Some("3".into()));
    }

    #[test]
    fn strchr_not_found() {
        assert_eq!(call_function("strchr", &s(&["hello", "z"])), Some("".into()));
    }

    #[test]
    fn strrchr_found() {
        assert_eq!(call_function("strrchr", &s(&["hello", "l"])), Some("4".into()));
    }

    #[test]
    fn strstr_found() {
        assert_eq!(call_function("strstr", &s(&["hello world", "world"])), Some("7".into()));
    }

    #[test]
    fn strstr_not_found() {
        assert_eq!(call_function("strstr", &s(&["hello", "xyz"])), Some("".into()));
    }
    ...
    #[test]
    fn strcmp_equal() {
        assert_eq!(call_function("strcmp", &s(&["hello", "hello"])), Some("1".into()));
    }

    #[test]
    fn strcmp_not_equal() {
        assert_eq!(call_function("strcmp", &s(&["hello", "HELLO"])), Some("".into()));
    }

    #[test]
    fn stricmp_equal() {
        assert_eq!(call_function("stricmp", &s(&["hello", "HELLO"])), Some("1".into()));
    }

    #[test]
    fn stricmp_not_equal() {
        assert_eq!(call_function("stricmp", &s(&["hello", "world"])), Some("".into()));
    }
    ...
    #[test]
    fn longer() {
        assert_eq!(call_function("longer", &s(&["hello", "hi"])), Some("1".into()));
        assert_eq!(call_function("longer", &s(&["hi", "hello"])), Some("".into()));
    ...
    #[test]
    fn if2_first() {
        assert_eq!(call_function("if2", &s(&["first", "second"])), Some("first".into()));
    }

    #[test]
    fn if2_fallback() {
        assert_eq!(call_function("if2", &s(&["", "second"])), Some("second".into()));
    }
    ...
    #[test]
    fn num_already_wide() {
        assert_eq!(call_function("num", &s(&["12345", "3"])), Some("12345".into()));
    }
    ...
    #[test]
    fn muldiv() {
        assert_eq!(call_function("muldiv", &s(&["10", "3", "2"])), Some("15".into()));
    }

    #[test]
    fn muldiv_by_zero() {
        assert_eq!(call_function("muldiv", &s(&["10", "3", "0"])), Some("".into()));
    }
    ...
    #[test]
    fn ext() {
        assert_eq!(call_function("ext", &s(&["track.flac"])), Some("flac".into()));
    }
    ...
    #[test]
    fn filename() {
        assert_eq!(call_function("filename", &s(&["track.flac"])), Some("track".into()));
    }
    ...
    fn char_function() {
        ...
        assert_eq!(call_function("char", &s(&["8226"])), Some("\u{2022}".into()));
    }
