...
- **The same timer cost a third of a core everywhere else it was used.** The three bars marking the playing row drove their *height* from a `TimelineView` at 30 Hz, and the seek bar sized its played portion with `frame(width:)` at 10 Hz. Height and width are layout, so each of those forty ticks a second marked the window dirty and AppKit answered with a full Auto Layout pass over the whole view tree — a third of a core to draw nine points of bar and one line, and it went on doing it while the window was minimised, because a `TimelineView` does not care whether anyone is looking. The bars are scaled by CoreAnimation now and the seek bar is drawn rather than sized; neither touches layout.

- **The spectrum analyser ran for clients that have no visualiser.** `Player::spawn` starts it unconditionally, so the macOS app paid for a 2048-point FFT and a fresh waveform allocation sixty times a second and dropped every frame on the floor. It now stands down a second after the last reader of its snapshot goes away, and picks back up within 250 ms of one returning.
