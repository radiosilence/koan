...
- **Picking something from the search dropdown landed you on the list instead.** Choosing an album sent you to the album grid, an artist to the artist list. Two faults, one after the other. `jump(to:)` set a section and a route in the same move, and a `NavigationStack` throws its path away when its root changes under it — which swapping the section does — so the stack answered the push by writing an empty path back over it. It moves in two now: the section lands, then the route is pushed onto a stack that has finished rebuilding. Separately, emptying the field after acting on a result submits it again, and an empty submit was treated as "search for nothing", which shows the results page; clearing the query then forgets that page, dropping you on whatever list was behind it. A blank field submitting is no longer a request for anything.

  The two-move jump costs a frame of the parent list before the push. The cure is a stack whose root never changes, which is [#300](https://github.com/radiosilence/koan/issues/300).
